#!/usr/bin/env python
"""
Exemplo básico de servidor A2A para um assistente de clima.

Este exemplo demonstra a implementação de um agente A2A que fornece
informações sobre clima e previsões meteorológicas.
"""
...
agent_card = AgentCard(
    ...
    skills=[
        ...
        AgentSkill(
            id="historico_clima",
            name="Histórico de Clima",
            description="Fornece dados históricos de clima para uma localidade"
        ),
        AgentSkill(
            id="alertas_meteorologicos",
            name="Alertas Meteorológicos",
            description="Alerta sobre condições climáticas extremas"
        )
    ]
)
...
async def process_message(task_id: str, message_data: Dict[str, Any]) -> Task:
    ...
    elif "tempo em Brasília" in text.lower():
        response_text = "Em Brasília hoje está ensolarado com temperatura de 30°C."
    elif "tempo em Curitiba" in text.lower():
        response_text = "Em Curitiba hoje está chuvoso com temperatura de 22°C."
...
if __name__ == "__main__":
    logger.info("Iniciando servidor A2A Assistente Meteorológico")
